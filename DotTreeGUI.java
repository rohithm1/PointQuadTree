 ...
 * @author Sanjana Goli and Rohith Mandavilli -- updated drawTree, test2, handleKeyPress
 */
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMouseMotion(int x, int y) {
		if (mode == 'q') { //mode to update query
			mouseX = x; mouseY = y;
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			else tree = new PointQuadtree<Dot>(new Dot(x,y),0,0,width, height);
			...
			//TODO: YOUR CODE HERE
			query = true;
			//checks what nodes lie in the circle when queryings
			found = tree.findInCircle(x, y, mouseRadius, query);
		}
		else {
	...
	private void test2() {
		tree = new PointQuadtree<Dot>(new Dot(250,350), 0,0,800,600); // start with A
		tree.insert(new Dot(470,90));  //b
		tree.insert(new Dot(390, 200)); //c
		tree.insert(new Dot(540,190)); //d
		tree.insert(new Dot(150,100)); //e
		tree.insert(new Dot(175,40));  //f
		tree.insert(new Dot(160,400)); //g
		tree.insert(new Dot(520,380)); //h
		tree.insert(new Dot(295, 500));//i
		
		
		int bad = 0;
		bad += testFind(150,80,25,6,3,1); 	// rect for A [B] [E [F]] [G] [H]; circle for A, E, F; find E
		bad += testFind(500,125,10,7,3,0);	// rect for A [B [C D]] [E] [G] [H]; circle for A, B, D; find none
		bad += testFind(250,350,60,9,7,1);	// rect for A [B [C D]] [E [F]] [G] [H [I]]; circle for A,B,C,D,G,H,I; find A
		bad += testFind(475,90,10,7,4,1);   // rect for A [B [C D]] [E] [G] [H]; circle for A, B, C, D; find B
	...
	public void draw(Graphics g) {
		...
		if (mode == 'q') {
			...
			//draws oval based on where the mouse is clicked
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		//updates level so that colors change based on children
		if(tree.hasChild(1)) {
